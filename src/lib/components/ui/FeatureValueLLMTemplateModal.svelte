<!-- Feature Value LLM Template Modal -->
<script lang="ts">
  import { featureValueTemplateStore } from '$lib/stores/featureValueTemplateStore';
  ...
  $: if (show && results && $featureValueTemplateStore) {
    template = $featureValueTemplateStore.generateTemplate(results);
  }
  ...
  >
    ...
    >
      ...
      <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center bg-gray-50">
        <div>
          <h2 class="text-xl font-semibold text-gray-900">Feature Value Analysis Template</h2>
          <p class="text-sm text-gray-500 mt-1">Copy this prompt to get AI insights about your feature value analysis</p>
        </div>
        <button
          on:click={() => show = false}
      ...
      <div class="px-6 py-4 border-t border-gray-200 bg-gray-50 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div class="text-sm text-gray-500 text-center sm:text-left">
          Use this template to get detailed insights about your feature value analysis
        </div>
